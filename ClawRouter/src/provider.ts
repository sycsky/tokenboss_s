...
export const blockrunProvider: ProviderPlugin = {
  id: "blockrun",
  label: "BlockRun",
  docsPath: "https://tokenboss.co/docs",
  aliases: ["br"],
  ...
  get models() {
    if (!activeProxy) {
      // Fallback: point at the configured upstream (TokenBoss backend or
      // BlockRun) so OpenClaw can load model definitions before the proxy
      // starts.
      return buildProviderModels(getTokenBossUpstream() ?? "https://blockrun.ai/api");
    }

...
async function main(): Promise<void> {
  ...
  if (args.partners) {
    ...
    if (args.partnersTest) {
      ...
      const apiBase = getTokenBossUpstream() ?? "https://blockrun.ai/api";
      for (const svc of PARTNER_SERVICES) {
  ...
  // Pull tier config from TokenBoss (no-op when not in TokenBoss mode). Await
  // the initial fetch so the first incoming request already uses remote
  // config; subsequent refreshes happen in the background.

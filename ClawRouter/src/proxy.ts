...
import { getModelsCached } from "./model-cache.js";

// Upstream URL resolution: TokenBoss env override wins, else BlockRun default.
  ...
  try {
    ...
    if (Array.isArray(choices) && choices.length > 0) {
      ...
      if (msg) {
        ...
        if (!hasContent && !hasToolCalls && !hasReasoning && finishReason === "stop") {
...
export async function startProxy(options: ProxyOptions): Promise<ProxyHandle> {
  ...
  const server = createServer((req: IncomingMessage, res: ServerResponse) => {
    void (async () => {
      ...
      // --- Handle /v1/models: cache-first, refresh from TokenBoss backend on miss ---
...
async function tryModelRequest(
  ...
  try {
    ...
    // Real streaming bodies can't be JSON.parse'd; the degradation checks only
    // make sense for buffered JSON payloads. Let SSE through and rely on the
    // downstream streaming parser to handle any errors.
...
async function proxyRequest(
  ...
  // Only consulted for non-streaming requests. The cache key intentionally
  // ignores the `stream` flag, and only non-streaming responses are written
  // back (see responseCache.set below) — so letting a streaming request hit
  // this would return buffered JSON with content-type application/json
  // instead of an SSE stream, which clients then render as garbage.
  const cacheKey = ResponseCache.generateKey(body);
  const reqHeaders: Record<string, string> = {};
  for (const [key, value] of Object.entries(req.headers)) {
    if (typeof value === "string") reqHeaders[key] = value;
  }
  if (!isStreaming && responseCache.shouldCache(body, reqHeaders)) {
  ...
  try {
    ...
    if (headersSentEarly) {
      ...
      const upstreamIsSSE = upstreamContentType.includes("event-stream");

      if (upstreamIsSSE && upstream.body) {
        // --- Real SSE pass-through (TokenBoss mode with stream:true preserved) ---
        // Parse each `data: ...` event, strip thinking tokens from delta.content,
        // capture usage from the final chunk, rewrite finish_reason when tool_calls
        // are present, and inject balance/downgrade notices as extra chunks. Raw
        // bytes of the re-emitted stream are collected into responseChunks for
        // the dedup cache, same as the JSON→SSE path below.
        let sawDone = false;
        let noticeInjected = false;
        const maybeInjectNotice = (): void => {
          if (noticeInjected) return;
          if (!balanceFallbackNotice && !budgetDowngradeNotice) return;
          ...
          const baseChunk = {
            ...
            model: actualModelUsed,
            system_fingerprint: null as string | null,
          };
          if (balanceFallbackNotice) {
            ...
            const data = `data: ${JSON.stringify(notice)}\n\n`;
            safeWrite(res, data);
        ...
        const handleEvent = (rawEvent: string): void => {
          ...
          if (dataLines.length === 0) {
            // Comment / heartbeat — pass through verbatim.
            const out = `${rawEvent}\n\n`;
            safeWrite(res, out);
            responseChunks.push(Buffer.from(out));
            return;
          }
          ...
          if (payload.trim() === "[DONE]") {
            ...
            safeWrite(res, done);
            responseChunks.push(Buffer.from(done));
            return;
          }
          ...
          if (usage && typeof usage === "object") {
            ...
            if (typeof u.completion_tokens === "number")
              responseOutputTokens = u.completion_tokens;
          }
          ...
          if (Array.isArray(choices)) {
            for (const c of choices) {
              ...
              if (delta && typeof delta.content === "string" && delta.content.length > 0) {
                const stripped = stripThinkingTokens(delta.content);
                if (stripped !== delta.content) {
                  delta.content = stripped;
                  mutated = true;
                }
                if (stripped) accumulatedContent += stripped;
              }
        ...
        try {
          ...
          while (true) {
            ...
            while (idx !== -1) {
              const event = lineBuffer.slice(0, idx);
              lineBuffer = lineBuffer.slice(idx + 2);
              handleEvent(event);
              idx = lineBuffer.indexOf("\n\n");
            }
          }
          if (lineBuffer.trim()) handleEvent(lineBuffer);
        } finally {
          reader.releaseLock();
        }

        // Fallback: upstream closed without [DONE] — emit cost + terminator.
        if (!sawDone) {
          if (routingDecision) {
            const costComment = `: cost=$${routingDecision.costEstimate.toFixed(4)} savings=${(routingDecision.savings * 100).toFixed(0)}% model=${actualModelUsed} tier=${routingDecision.tier}\n\n`;
            safeWrite(res, costComment);
      ...
      // SSE pass-through branch above already emitted cost comment, [DONE],
      // ended the response, and registered the dedup entry — only run the
      // buffered JSON→SSE tail logic when we took that legacy path.
      if (!upstreamIsSSE) {
      ...
      if (debugMode && routingDecision) {
        ...
        // HTTP header values must be latin-1 / ASCII; reasoning may contain
        // non-ASCII chars (e.g. CJK from prompt echo), which crashes Node's
        // header writer. Strip or URL-encode so debugging still works.
        responseHeaders["x-clawrouter-reasoning"] = encodeURIComponent(

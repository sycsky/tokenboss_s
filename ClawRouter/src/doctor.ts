...
/** Upstream API base — TokenBoss if configured, otherwise BlockRun. */
const UPSTREAM_API = getTokenBossUpstream() ?? "https://blockrun.ai/api";
...
async function collectNetworkInfo(): Promise<NetworkInfo> {
  ...
  try {
    ...
    const response = await fetch(`${UPSTREAM_API}/v1/models`, {
      method: "GET",
...
async function analyzeWithAI(
  ...
  try {
    ...
    const response = await paymentFetch(`${UPSTREAM_API}/v1/chat/completions`, {
      method: "POST",

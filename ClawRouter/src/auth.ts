...
import { isTokenBossMode, TOKENBOSS_DUMMY_WALLET_KEY } from "./tokenboss.js";
...
export async function resolveOrGenerateWalletKey(): Promise<WalletResolution> {
  // TokenBoss fork mode: skip wallet generation entirely.
  // The dummy key lets privateKeyToAccount() succeed for downstream code
  // that still reads account.address, but no signing ever happens because
  // the x402 code path is bypassed in proxy.ts.
  if (isTokenBossMode()) {
    const account = privateKeyToAccount(TOKENBOSS_DUMMY_WALLET_KEY as `0x${string}`);
    console.log(`[ClawRouter] TokenBoss fork mode — bypassing wallet generation`);
    return {
      key: TOKENBOSS_DUMMY_WALLET_KEY,

...
function injectModelsConfig(logger: { info: (msg: string) => void }): void {
  ...
  // Routing mode:
  //   proxy  (default) — openclaw → local :8402 proxy → TokenBoss backend.
  //                      Routing (auto/eco/premium/agentic → concrete model)
  //                      happens locally in this plugin. Zero trust in backend.
  //   direct          — openclaw → TokenBoss backend directly; backend does
  //                      the routing. Saves a hop, but requires trusting the
  //                      backend's router decisions.
  //
  // Opt in with TOKENBOSS_ROUTER_MODE=direct (or legacy TOKENBOSS_ROUTER_DIRECT=1).
  const directMode =
    (process.env.TOKENBOSS_ROUTER_MODE === "direct" ||
      process.env.TOKENBOSS_ROUTER_DIRECT === "1") &&
    !!process.env.TOKENBOSS_API_URL &&
    !!process.env.TOKENBOSS_API_KEY;

  const proxyPort = getProxyPort();
  const expectedBaseUrl = directMode
    ? `${process.env.TOKENBOSS_API_URL!.replace(/\/+$/, "")}/v1`
    : `http://127.0.0.1:${proxyPort}/v1`;
  const expectedApiKey = directMode
    ? process.env.TOKENBOSS_API_KEY!
    : "x402-proxy-handles-auth";
  ...
  if (!providers.blockrun) {
    ...
    // In direct mode, sync the apiKey to the user's current sk-xxx (they may
    // have rotated it). In proxy mode, keep the placeholder.
    if (blockrun.apiKey !== expectedApiKey) {
      blockrun.apiKey = expectedApiKey;
      fixed = true;
    }
 ...
 */
// ── Watchdog ──────────────────────────────────────────────────────────
// Pings /health every 30s; after 2 consecutive failures assumes the proxy
// died (crash, silent listener drop, someone killed the process on port
// 8402) and reboots it. One-shot; the interval is idempotent.

let watchdogTimer: NodeJS.Timeout | null = null;
let watchdogFailureStreak = 0;
let watchdogRebooting = false;
const WATCHDOG_INTERVAL_MS = 30_000;
const WATCHDOG_FAIL_THRESHOLD = 2;
const WATCHDOG_REQUEST_TIMEOUT_MS = 3_000;

function startWatchdog(api: OpenClawPluginApi): void {
  if (watchdogTimer) return; // already running — register() may be called multiple times
  watchdogTimer = setInterval(() => {
    void tickWatchdog(api);
  }, WATCHDOG_INTERVAL_MS);
  // Don't keep the event loop alive just for health checks
  watchdogTimer.unref?.();
  api.logger.info(
    `Watchdog started (interval=${WATCHDOG_INTERVAL_MS / 1000}s, fail-threshold=${WATCHDOG_FAIL_THRESHOLD})`,
  );
}

async function tickWatchdog(api: OpenClawPluginApi): Promise<void> {
  if (watchdogRebooting) return;
  ...
  if (ok) {
    if (watchdogFailureStreak > 0) {
      api.logger.info(`Watchdog: proxy recovered after ${watchdogFailureStreak} failure(s)`);
    }
  ...
  api.logger.warn(
    `Watchdog: /health failed (streak=${watchdogFailureStreak}/${WATCHDOG_FAIL_THRESHOLD})`,
  );
  if (watchdogFailureStreak < WATCHDOG_FAIL_THRESHOLD) return;

  watchdogRebooting = true;
  try {
    api.logger.warn(`Watchdog: rebooting proxy on port ${port}`);
    if (activeProxyHandle) {
      try {
        await activeProxyHandle.close();
      } catch (err) {
        api.logger.warn(`Watchdog: close() threw (continuing): ${String(err)}`);
      }
      activeProxyHandle = null;
    }
    await startProxyInBackground(api);
    watchdogFailureStreak = 0;
    api.logger.info(`Watchdog: proxy rebooted successfully`);
  } catch (err) {
    api.logger.error(`Watchdog: reboot failed: ${String(err)}`);
  } finally {
    watchdogRebooting = false;
  }
}

async function startProxyInBackground(api: OpenClawPluginApi): Promise<void> {
  // Pull tier mapping from TokenBoss backend before anything routes.
  // Non-fatal — falls back to compiled defaults on failure.
  try {
    await startRemoteTiersRefresh();
    api.logger.info(`Remote tier config refresh started`);
  } catch (err) {
...
const plugin: OpenClawPluginDefinition = {
  id: "tokenboss-router",
  name: "TokenBoss Router",
  description: "TokenBoss edge router — local LLM proxy for TokenBoss relay (fork of ClawRouter).",
  version: VERSION,
  ...
  register(api: OpenClawPluginApi) {
    ...
    const logProc = proc as NodeJS.Process & { __clawrouterRegisterLogged?: boolean };
    const firstTime = !logProc.__clawrouterRegisterLogged;
    if (firstTime) {
    ...
    api.registerService({
      ...
      stop: async () => {
        ...
        // Critical: clear the proc-level flag so a reloaded plugin (same Node
        // process — openclaw gateway restart doesn't kill the process) can
        // start a fresh proxy. Without this, the next register() sees the
        // stale "already started" flag and silently skips startup.
        const p = process as NodeJS.Process & {
          __clawrouterProxyStarted?: boolean;
          __clawrouterProxyStarting?: boolean;
        };
        p.__clawrouterProxyStarted = false;
        p.__clawrouterProxyStarting = false;
      },
    });
    ...
    // Don't short-circuit on proxyAlreadyStarted alone — openclaw gateway
    // restart reloads plugins in the SAME Node process, so the flag from an
    // old plugin version can leak into a freshly loaded new version. Only
    // trust the flag when we also still hold an activeProxyHandle (i.e.
    // this plugin module actually owns the running proxy). The portProbe
    // further down then serves as the async ground-truth check.
    if (proxyAlreadyStarted && activeProxyHandle) {
      return;
    }
    if (proxyAlreadyStarted && !activeProxyHandle) {
      api.logger.warn(
        `Stale __clawrouterProxyStarted flag detected (no active handle) — will attempt restart`,
      );
      proc.__clawrouterProxyStarted = false;
    }

    // Direct mode: when running as an openclaw plugin against a TokenBoss
    // backend, skip the local 127.0.0.1:8402 proxy entirely. The blockrun
    // provider already falls back to getTokenBossUpstream() when no proxy
    // handle is set, so requests go straight from openclaw to the TokenBoss
    // backend. Standalone CLI (cli.ts) bypasses this file, so `npx
    // tokenboss-router` still starts the local proxy as usual.
    // Default = proxy mode (routing happens locally in this plugin).
    // Opt into direct mode with TOKENBOSS_ROUTER_MODE=direct (or legacy
    // TOKENBOSS_ROUTER_DIRECT=1) — that tells openclaw to bypass our proxy and
    // hit the TokenBoss backend itself, which then runs the virtual-model
    // resolver. Keeping default=proxy because most users want local routing
    // for trust reasons; standalone CLI always proxies.
    const directMode =
      (process.env.TOKENBOSS_ROUTER_MODE === "direct" ||
        process.env.TOKENBOSS_ROUTER_DIRECT === "1") &&
      !!process.env.TOKENBOSS_API_URL &&
    ...
    if (directMode) {
      api.logger.info(
        `TokenBoss direct mode — skipping local proxy; provider will call ${process.env.TOKENBOSS_API_URL} directly (backend routes virtual models)`,
      );
      proc.__clawrouterProxyStarted = true;
      return;
    }

    // Don't set __clawrouterProxyStarted yet — only after startProxy actually
    // succeeds. Setting it eagerly meant a startup failure permanently locked
    // out further attempts (port stayed dead, no recovery).
    const procEx = proc as NodeJS.Process & { __clawrouterProxyStarting?: boolean };
    if (procEx.__clawrouterProxyStarting) {
      api.logger.info("Proxy startup already in flight — skipping");
      return;
    }
    procEx.__clawrouterProxyStarting = true;
    ...
    portProbe
      .then((portInUse) => {
        if (portInUse) {
          ...
          // Treat as success: someone owns the port, no point retrying in this proc.
          proc.__clawrouterProxyStarted = true;
          return;
        }
        return startProxyInBackground(api).then(async () => {
          ...
          if (!healthy) {
            ...
            // Don't set started — let the next register() retry. Watchdog also
            // attempts recovery if it ever started.
            return;
          }
          proc.__clawrouterProxyStarted = true;
        });
      ...
      .catch((err) => {
        ...
        // Leave __clawrouterProxyStarted=false so the next register() retries.
      })
      .finally(() => {
        procEx.__clawrouterProxyStarting = false;
      });
  },
